<template>
    ...
    <Toolbar :info="info" :idTable="tableId" :tools="toolBarButton" :bulk="bulkActions"/>
      ...
      <template v-slot:cell(action)="row">
        <div class="actions--menu" v-if="['ROLE_ADMIN'].includes($store.getters['auth/user'].role)">
          ...
          <div class="action--menu">
            <b-button
                ...
                size="sm"
                variant="link"
                @click="enroll(row.item.id, $store.getters['auth/user'].id)"
            >
              <font-awesome-icon class="action--item action-delete text-black-50" icon="user-plus" />
            </b-button>
          </div>
          <div class="action--menu" >
            <b-button
        ...
        <div class="actions--menu" v-if="['ROLE_STUDENT'].includes($store.getters['auth/user'].role)">
          <div class="action--menu">
            <b-button
                v-b-tooltip.hover
                title="Iscriviti al Corso"
	...
	export default {
      ...
      FormScore,
        ...
        fieldsRoll    : 'courses/getFieldsRoll',
			...
			...mapState({
				tableId      : state => state.courses.tableId,
				toolBarButtonUser: state => state.courses.toolBarButton,
        toolBarButtonAdmin: state => state.courses.toolBarButtonAdmin,
				apiModule    : state => state.courses.apiModule,
				storeModule  : state => state.courses.storeModule,
        apiModuleScore: state => state.courses.apiModuleScore,
			}),
		},
		methods: {
      ...
      if(['ROLE_ADMIN'].includes(this.$store.getters['auth/user'].role)) {
        ...
        this.toolBarButton = this.toolBarButtonUser
      }
			this.$nextTick(() => {
				this.provider();
			})
